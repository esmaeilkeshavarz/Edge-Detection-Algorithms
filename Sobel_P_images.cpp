...
int main()
{
    ...
    double start,end;
    start = omp_get_wtime();
    int chunk_size= (_src.rows*_src.cols)/20; // total_num_iterations/10*2

        #pragma omp parallel for  default(shared)  private(r,c,i,j,s) num_threads(4) ordered schedule(dynamic,chunk_size) collapse(2) 
